...
<div class="container">
  ...
  <form id="lionForm" name="lion-report" autocomplete="off">
  ...
  <div id="success">
    ...
    <h2>Submitted</h2>
    <p>A new ClickUp task was created for review. The bot will append this week's data automatically when wired up.</p>
    <p style="margin-top: 16px;"><a id="taskLink" href="#" target="_blank" style="color: var(--accent-light); text-decoration: underline;">View the task in ClickUp →</a></p>
  </div>
</div>
...
<script>
  ...
  // Submit handler: POST to Netlify Function, then show success
  ...
  form.addEventListener('submit', async (e) => {{
    ...
    // Collect form data; treat unchecked checkboxes as missing
    ...
    try {{
      const resp = await fetch('/.netlify/functions/submit-lion', {{
        method: 'POST',
      ...
      if (result.taskUrl) {{
        document.getElementById('taskLink').href = result.taskUrl;
      }}
    }} catch (err) {{
      errorBox.textContent = 'Error: ' + err.message + '. Your data was NOT saved. Please try again or report to Hymie.';
      errorBox.style.display = 'block';
      submitBtn.disabled = false;
      submitBtn.textContent = 'Submit LION Report';
    }}
  }});
</script>
</body>
